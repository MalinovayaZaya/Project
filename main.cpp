...
if(e==0){
    ifstream A; //Траблы строчкой ниже!
    A.open(C:\Users\Сергей\Documents\Полина\Документы\C++\Проект с текстом); //Здест необходимо ввест ссылку на текстовый файл из компьютера (D:\\sites\\accounts.txt)
  if(A){
    for (int i = 0; i < NUM_VERTICES; ++i){
		for (int j = 0; j < NUM_VERTICES; ++j){
    while(!A.eof()){
